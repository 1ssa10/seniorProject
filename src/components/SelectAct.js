...
function SelectAct({ setSelected }) {
  ...
  const selectactorhandler = (actor, e, index) => {
    e.preventDefault();
    // console.log(actor);
    // console.log(index);
    // setSelectedActors([...selectedactor, { ...actor, selected: true }])
    // console.log(selectedactor);
    setSelectedActors((prevSelected) => {
      // Check if the actor is already selected
      const actorIndex = prevSelected.findIndex(
        (selectedActor) => selectedActor.id === actor.id
      );

      // If the actor is not selected, add it to the selected actors list
      if (actorIndex === -1) {
        return [...prevSelected, { ...actor, selected: true }];
      } else {
        // If the actor is selected, remove it from the selected actors list
        const updatedSelected = [...prevSelected];
        updatedSelected.splice(actorIndex, 1);
